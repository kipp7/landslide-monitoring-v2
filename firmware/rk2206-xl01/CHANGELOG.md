...
## [2026-07-24] - 紧凑广播轮询与三节点错峰回传实验

- 新增固定 `10` 字节 `P1xxxxxxxx` 广播轮询命令，field-link 线上总长 `28` 字节。
- A/B/C 收到同一命令后分别在 `0/340/680 ms` 时隙回传，避免同信道同时发射碰撞。
- 每秒只需要一条下行轮询命令，继续使用 `64` 字节紧凑遥测回包。
