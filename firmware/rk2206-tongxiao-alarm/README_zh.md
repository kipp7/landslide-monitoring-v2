...
- LCD 使用套件 `landslide_monitor` Sample 的 UTF-8 中文字库。待机页显示“设备正常 / 风险等级正常”；告警页只显示结构化的监测点、等级、固定撤离建议和告警编号，不直接渲染可能缺字的服务端标题与信息。没有当前告警时显示设备 ID 后 8 位、短固件版本、告警指令序号和设备状态；解除消息保留的历史告警上下文不会继续显示撤离建议。WIFI、MQTT、UUID 等协议值保留 ASCII，固定标签尽可能使用中文。
