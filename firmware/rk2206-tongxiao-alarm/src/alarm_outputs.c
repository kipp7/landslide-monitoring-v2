...
void AlarmOutputs_Tick(const AlarmDesiredState *desired, uint32_t phase_ms)
{
    ...
    if (desired->state == ALARM_STATE_SILENCED) {
        SetPwm(BUZZER_PWM, false, 50, 2000, &g_buzzer_on);
        SetPwm(MOTOR_PWM, false, 70, 1000, &g_motor_on);
    ...
    pulse_on = (phase_ms % period_ms) < (period_ms / 2U);
    SetPwm(BUZZER_PWM, desired->buzzer && pulse_on, 50, 2000, &g_buzzer_on);
    SetPwm(MOTOR_PWM, desired->motor && pulse_on, 70, 1000, &g_motor_on);
