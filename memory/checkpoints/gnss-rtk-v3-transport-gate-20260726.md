...
- 早期未校准 hardware 预检包 `xls1_rs485_hardware_preflight_uncalibrated_20260802` 已被最终校准包取代，不再烧录。最终包由干净 detached worktree 的 `82585ec58843f44d3b6e6d4f9a3f0082ffdf4b45` 构建到 `F:\2\openharmony\rk2206_firmware_releases\xls1_rs485_hardware_preflight_calibrated_20260802`；manifest 为 hardware、RS485 initialized、RTCM disabled、A/B/C 均 `field-calibrated`，校准来源 SHA-256 为 `678f7c36e160e1e4259367ea40690dd4e87369b9c36ee77ac918d32f4192a87b`。A/B/C `.img` SHA-256 分别为 `e30ff4949a246110ee33bc77b3e006bcf453f3bce99a931bb95cc407a5e859d5`、`a068a6b9c4a4ac8fad3426a8eb022ae2c813c846a2e99e19b8d7b679cc9b7bf8`、`00c6593eac439da9cd8e9579ad29907bf66ec0c5dc8fdd53cd71a154997a0212`。包内原样 `verify-rk2206-release-safety.ps1` 命令已返回 `RELEASE_SAFETY_OK`（7 个文件、A/B/C、hardware、RTCM disabled、field-calibrated、指定提交）；共享 SDK 同步文件由 `try/finally` 恢复、临时备份目录为 0、构建 worktree 保持 clean。`DO-NOT-FLASH-UNTIL-RS485-INSTALLED.txt` 仍是硬门禁：接口安装及断电/首次上电检查完成前禁止烧录。
...
- RS485 元件未到期间，模拟包故意不初始化 PB4/PB5。最终已校准 hardware 预检包已经生成并通过发布门禁，但仍设为 `DO-NOT-FLASH`；接口到货、焊接与断电/首次上电电气检查完成后才允许按物理身份烧录，不可手工删除模拟函数、修改 XLS1 驱动或绕过包内检查顺序。
...
继续 2026-08-02 XLS1/RTK 链路任务：生产链保持 1000 ms 冷却、1200 ms 首响应窗、部分响应时同标签最多重发一次和 2500 ms 总会话；最终 1800 秒为 2787/2787、零错误。A/B/C 的 `VBAT_SW` 中性采集和逐节点校准已完成，增益为 `1046565/1056299/984111 ppm`；先按身份烧录 `F:\2\openharmony\rk2206_firmware_releases\xls1_link_rehearsal_battery_calibrated_20260802` 并复测每节点误差不超过 60 mV、质量为 `field-calibrated`。RS485 接口到货后严格执行 `F:\2\openharmony\rk2206_firmware_releases\xls1_rs485_hardware_preflight_calibrated_20260802\DO-NOT-FLASH-UNTIL-RS485-INSTALLED.txt`，通过断电和首次上电门禁后才烧对应 hardware 镜像；随后完成至少 600 秒真实传感器门禁，最后才恢复 RTCM PROBE/真实 NTRIP。原始报告、坐标和凭据不进入 Git。
