...
- 倾角属于 1 秒核心路径，改用独立 `300 ms` 响应超时和最多 1 次重试；计入驱动每次
  `50 ms` TX-complete 等待与 `80 ms` 重试间隔，最坏等待约 `780 ms`。环境土壤/EC
  仍为 `300 ms/0 retry`，原有 800 ms 仅保留给非核心雨量等路径。这样单次倾角无响应
  不会拖过下一个核心采样槽，同时保留一次瞬态恢复机会。
