---
title: gnss-rtk-correction-and-antenna-strategy
type: note
tags:
  - decision
  - gnss
  - rtk
  - rk3568
  - rk2206
status: active
---

# Decision: gnss-rtk-correction-and-antenna-strategy

## Context

系统有 3 个 RK2206 现场节点，每个节点连接 1 个 UM220-IV NK，节点通过 DL-XLS1/XL01 与 RK3568 网关通信。目标是在不立即更换全部接收机的前提下提高定位精度，并验证现有 NK 是否能够稳定进入 RTK Fixed。

PC 端已证明千寻知寸 FindCM、NTRIP 链路和 RTCM 注入路径可用。现有普通天线只能稳定达到 RTK Float，尚未出现 RTK Fixed，因此当前优先瓶颈指向天线及现场信号质量，但不能排除接收机、固件、多路径和馈线因素。

## Decision

- RK3568 作为唯一 NTRIP 客户端，获取原始二进制 RTCM 后广播给 3 个 RK2206 节点。
- RK2206 将 RTCM 原样转发到各自 UM220-IV NK 的 RX；不得使用 JSON 或 Base64 包装 RTCM。
- 在现有 COBS + CRC 链路协议中增加独立的二进制 RTCM 帧类型。
- 先采购 1 个北天 BT-760 和 1 条 3 m、50 ohm、TNC-J 到 SMA-J、非 RP-SMA 馈线进行 A/B 验证；通过后再购买另外 2 套同型号、尽量同批次的天线与馈线。
- 不为缺少可量化差异的 BT-760E 支付溢价。卖家表中 BT-760 与 BT-760E 的公开参数相同，北天官网当前可查到 BT-760 正式产品页，但未查到能证明 E 版本更优的正式规格。
- 只有在天线、馈线、开阔度、差分龄和固件均验证正常而 BT-760 仍无法稳定 Fixed 后，才评估将 rover 升级为 UM960 或 UM982。
- 坐标存储和输出使用 `double`，至少保留 8 位小数，并同时保留 GGA 质量、卫星数、HDOP 和差分龄；不得只上传截断后的经纬度。
- HarmonyOS 地图优先消费 RK3568/API 的 `rtk_latitude_deg` / `rtk_longitude_deg` 完整坐标对；迁移期间继续兼容 `latitude_deg` / `longitude_deg`、`gnss_latitude` / `gnss_longitude` 和旧的 `gps_latitude` / `gps_longitude`，不得混用不同来源的纬度和经度。
- HarmonyOS 站点页复用每 10 秒已拉取的设备状态更新地图坐标，不为地图重复请求同一个 `/data/state`；设备离线或定位字段暂时缺失时使用最后一次有效位置。

## Rationale

- 单个 BT-760 的试错成本约为人民币 130 元，明显低于一次性更换 3 个接收机。
- BT-760 官方标称四星全频、38 +/- 2 dB 增益、轴比不大于 3 dB、相位中心误差 +/- 2 mm、TNC-K 接口，并采用多馈点对称结构和抗多路径扼流设计，方向上符合 RTK 天线需求。
- 已有测试显示更好的天线能把首次 Float 从约 259 s 缩短到约 3.4 s，并将 Float 占比提高到约 85%，说明天线质量对当前系统有显著影响。
- 单一 NTRIP 客户端降低账号连接数、移动网络流量和运维复杂度，并与现有 RK3568 到 RK2206 的星型链路一致。

## Consequences

- 在 BT-760 通过验收前，不批量采购 3 套，也不立即实施三节点 RTCM 广播。
- BT-760 是合理的首选验证天线，但官方标称不等于现场厘米级保证；RTK Fixed 仍取决于天空视野、多路径、差分质量、馈线和接收机算法。
- 升级接收机不能修复严重遮挡、多路径、错误馈线或过期差分数据，因此模块升级不是默认兜底。
- UM220-IV NK 按 rover 使用；当前测试未得到有效基站 RTCM 输出，不把它规划为 Base。
- App 的厦门大学坐标只作为没有任何实时、历史或站点坐标时的明确默认位置，不能标记为实时 GPS 或 RTK。

## Follow-up

- 到货后在同一开阔位置运行 30 至 60 分钟对比测试。
- 通过条件：约 5 分钟内出现 GGA=4，Fixed 占比高于 80%（优选高于 90%），差分龄保持 1 至 3 s，且不频繁回退到 GGA=2/5。
- 通过后采购另外 2 套同批次天线和馈线；失败时先完成信号链排查，再决定是否更换 UM960/UM982。
- 服务器部署 RTK 字段后，用 A/B/C 三个节点分别验证 `/api/v1/data/state/{deviceId}` 和七日曲线能返回成对坐标，再在 HarmonyOS 模拟器确认节点位置会自动移动且离线节点保持最后位置。
