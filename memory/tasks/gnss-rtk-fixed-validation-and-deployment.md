...
- A/B/C 电池单点校准已全部验收：最终增益分别为 `1046565/1048458/993702 ppm`，绝对误差分别为 `+9/+7/最坏 9 mV`，全部满足 `<=60 mV` 门槛。现有旧 hardware 包仍含过期校准并继续禁止烧录；下一步是以这组三节点最终值重建并独立验证最终 simulated/hardware 发布包。
...
- A/B/C 已分别以 `+9/+7/最坏 9 mV` 通过电池同步验收并接受 `1046565/1048458/993702 ppm`。下一项是用最终三节点值重建并验证 simulated/hardware 发布包；百分比仍只是受负载、温度和老化影响的 3S OCV 估算，不能作为准确剩余 mAh 或续航。
