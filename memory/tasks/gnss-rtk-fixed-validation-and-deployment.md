...
- 从干净提交 `b084f10aaa47edd4e6039d2a422b441197c84f8f` 构建待烧录的校准模拟包 `F:\2\openharmony\rk2206_firmware_releases\xls1_link_rehearsal_battery_calibrated_20260802`。manifest 为 `sourceDirty=false`、RS485 simulated、`rs485HardwareInitialized=false`、RTCM disabled、三节点 `field-calibrated`；发布验证器、PC0 引脚安全门禁和校准生成器回归均通过。A/B/C `.img` SHA-256 分别为 `92af5fded3b3c71865a688e6e4e4b3a81f114785b41f9b7cf79471c149bbd10a`、`d10a4b3a171141c4647179b3fa6f0ae707b18ebb274c93414e3e7199dca1dfff`、`25d2184c4efc63f7505a05e2e730d19a288f14137e2285c6fcbc985012732434`。该包仍不初始化 PB4/PB5；烧录后必须重新同步测量并满足每节点误差不超过 60 mV、质量为 `field-calibrated`，构建成功不等于现场校准已验收。
- 最终已校准 RS485 hardware 预检包由干净提交 `82585ec58843f44d3b6e6d4f9a3f0082ffdf4b45` 构建到 `F:\2\openharmony\rk2206_firmware_releases\xls1_rs485_hardware_preflight_calibrated_20260802`。manifest 为 `sourceDirty=false`、hardware、`rs485HardwareInitialized=true`、RTCM disabled、A/B/C 均 `field-calibrated`，校准来源 SHA-256 为 `678f7c36e160e1e4259367ea40690dd4e87369b9c36ee77ac918d32f4192a87b`；A/B/C `.img` SHA-256 分别为 `e30ff4949a246110ee33bc77b3e006bcf453f3bce99a931bb95cc407a5e859d5`、`a068a6b9c4a4ac8fad3426a8eb022ae2c813c846a2e99e19b8d7b679cc9b7bf8`、`00c6593eac439da9cd8e9579ad29907bf66ec0c5dc8fdd53cd71a154997a0212`。包内原样验证命令已返回 `RELEASE_SAFETY_OK`；构建器恢复共享 SDK 后无 `xls1-compact-sdk-backup-*` 遗留，detached worktree 保持 clean。接口元件未安装、断电连续性/短路/方向检查和首次上电 3.3 V/PB4/PB5 检查未通过前，必须遵守 `DO-NOT-FLASH-UNTIL-RS485-INSTALLED.txt`，禁止烧录该包。
...
- A/B/C 的中性固件逐节点万用表采集和校准包构建已完成；下一项现场工作是按物理身份烧录校准模拟包并复测，要求每节点电压中位数与同步万用表值相差不超过 60 mV、质量为 `field-calibrated`。百分比仍只是受负载、温度和老化影响的 3S OCV 估算，不能作为准确剩余 mAh 或续航。
