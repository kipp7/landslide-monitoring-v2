...
- 审计发现旧 `xls1_three_node_batch_poll.py` 只识别 46 B V1/V2，无法验收 95/139 B V3/V4；已由提交 `97473b62` 补齐与 RK3568 一致的 V3/V4 定点解码、可信 RTK 约束、硬件土壤/EC/三轴倾角范围、field-calibrated PC0 和 RTCM fail-closed/零历史活动门禁。新增 `xls1_compact_v4_acceptance.py` 一次持有网关服务，按 60/600/1800 秒失败即停、逐段原子写 JSON 和 SHA-256，并在任何退出路径恢复服务；完整规程见 `docs/field-tests/rk2206-compact-v4-hardware-acceptance.md`。
- 两个验收脚本已部署到 RK3568 `/usr/local/bin`，SHA-256 分别为 `5ca27b1164f3bf61ff67ecc8461c1a00db34928718c55643dbf056469735e5e5`、`d29b5171ca64c05507196920f436202c6d7b98fe79a83fa60858b8c17ef4a7ba`，回滚目录 `/home/linaro/lsmv2-backups/field-test-scripts-97473b62-20260803-031211`。仅执行了无发送 `--check-prerequisites`：环境文件 `0600 root:root`、NTRIP false、`/dev/ttyS3` 为字符设备、服务 active 且 `NRestarts=0`；未向已下电节点发送验收轮询。
