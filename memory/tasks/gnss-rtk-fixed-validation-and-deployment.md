...
- 暂停累计 ACK 和 12 s/60 s RTCM 门禁。B/C 均已通过交叉试验定位到核心板侧；先以 A 为电气基准比较三块板在同一正常位置的 SDA/SCL 空闲电压，断电检查 PB4/PB5 到排针的连续性、对 3.3 V/GND 阻值、弯针/虚焊/污染。若无显而易见且可稳定修复的问题，比赛路径直接更换 B/C 核心板并重新烧录各自 V4 身份固件。只有三节点 U4 自检全 0、`LSR=0x60` 且传感器恢复后，才继续 ACK 和真实混合负载；此前保持 `LIVE` 关闭。
