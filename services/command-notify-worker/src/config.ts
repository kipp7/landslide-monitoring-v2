...
const configSchema = z.object({
  serviceName: z.string().default("command-notify-worker"),

  kafkaBrokers: z
    .string()
    .min(1)
    .transform((v) => v.split(",").map((s) => s.trim()).filter(Boolean)),
  kafkaClientId: z.string().default("command-notify-worker"),
  kafkaGroupId: z.string().default("command-notify-worker.v1"),
