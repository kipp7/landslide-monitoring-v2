...
- 对带高频 `packet_class` 的消息，writer 会按 `HIGH_FREQUENCY_BUDGET_BYTES` 检查服务器端序列化 JSON 语义包；该值不是 XLS1 空口紧凑帧预算。超限时消息会写入 `telemetry.dlq.v1`，不再写 ClickHouse，也不再更新 `device_state`。
