...
const configSchema = z.object({
  serviceName: z.string().default("telemetry-dlq-recorder"),

  kafkaBrokers: z
    .string()
    .min(1)
    .transform((v) => v.split(",").map((s) => s.trim()).filter(Boolean)),
  kafkaClientId: z.string().default("telemetry-dlq-recorder"),
  kafkaGroupId: z.string().default("telemetry-dlq-recorder.v1"),
