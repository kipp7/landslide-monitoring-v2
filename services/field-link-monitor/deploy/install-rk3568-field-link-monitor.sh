...
usage() {
  cat <<'EOF'
Usage:
  sudo bash services/field-link-monitor/deploy/install-rk3568-field-link-monitor.sh [options]
...
if [[ ! -d "${REPO_ROOT}/services/field-link-monitor" ]]; then
  echo "Service directory not found under repo root: ${REPO_ROOT}/services/field-link-monitor" >&2
  exit 1
fi
...
if [[ "${BUILD_FIRST}" -eq 1 ]]; then
  ...
  runuser -u "${RUN_USER}" -- env PATH="${PATH}" bash -lc "
    cd \"${REPO_ROOT}\"
    rm -rf \
      \"${REPO_ROOT}/services/field-link-monitor/dist\" \
...
sed \
  ...
  -e "s|__WORKING_DIRECTORY__|${REPO_ROOT}/services/field-link-monitor|g" \
...
echo "Working directory: ${REPO_ROOT}/services/field-link-monitor"
