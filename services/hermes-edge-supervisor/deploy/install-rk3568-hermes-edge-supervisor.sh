...
DIAGNOSIS_MODEL_PATH="${DIAGNOSIS_MODEL_PATH:-${REPO_ROOT}/services/hermes-edge-supervisor/models/edge-diagnosis-rf-v1.json}"
...
usage() {
  cat <<'EOF'
Usage:
  sudo bash services/hermes-edge-supervisor/deploy/install-rk3568-hermes-edge-supervisor.sh [options]
...
if [[ ! -d "${REPO_ROOT}/services/hermes-edge-supervisor" ]]; then
  echo "Service directory not found under repo root: ${REPO_ROOT}/services/hermes-edge-supervisor" >&2
  exit 1
fi
...
if [[ "${BUILD_FIRST}" -eq 1 ]]; then
  ...
  runuser -u "${RUN_USER}" -- env PATH="${PATH}" bash -lc "
    cd \"${REPO_ROOT}\"
    rm -rf \
      \"${REPO_ROOT}/services/hermes-edge-supervisor/dist\" \
...
sed \
  ...
  -e "s|__WORKING_DIRECTORY__|${REPO_ROOT}/services/hermes-edge-supervisor|g" \
...
echo "Working directory: ${REPO_ROOT}/services/hermes-edge-supervisor"
