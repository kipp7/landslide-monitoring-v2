...
const RealtimeSensorStatus = () => {
  ...
  const nodeConfigs = {
    'device_1': {
      nodeName: '龙门滑坡监测站',
      location: '防城港华石镇龙门村',
      sensorTypes: ['temperature', 'humidity', 'acceleration', 'illumination']
    },
    'device_2': {
      nodeName: '山体位移监测站',
      location: '玉林师范学院东校区',
      sensorTypes: ['temperature', 'humidity', 'gyroscope', 'vibration']
    },
    'device_3': {
      nodeName: 'GPS形变监测站',
      location: '广西南宁市',
      sensorTypes: ['temperature', 'acceleration', 'illumination', 'gps']
    }
  }
  ...
  const nodesData = useMemo(() => {
    ...
    Object.entries(nodeConfigs).forEach(([deviceId, config]) => {
      // 获取该节点的最新数据
      const nodeData = data
