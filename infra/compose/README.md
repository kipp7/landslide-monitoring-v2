...
- `docker-compose.app.yml`：应用层服务（API、采集/入库、规则与命令链路；旧 Web 仅在 `legacy-web` profile 下启用）
...
2. 修改密码与数据目录：
   ...
   - 如果后端开启登录，还需要设置 `JWT_ACCESS_SECRET`、`JWT_REFRESH_SECRET`
