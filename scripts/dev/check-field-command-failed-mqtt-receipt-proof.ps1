...
param(
  ...
  [string]$OutFile = "docs/unified/reports/field-command-failed-mqtt-receipt-proof-latest.json"
)
...
function Publish-MqttFailedAck([string]$MqttUrl, [string]$DeviceId, [string]$DeviceSecret, [string]$CommandId, [string]$ErrorMessage) {
  $nodeScript = @"
...
"@
  ...
  if ($LASTEXITCODE -ne 0) {
    throw "Failed to publish MQTT command failed ack"
  }
}
...
try {
  ...
  @(
    ...
    "KAFKA_CLIENT_ID=command-ack-receiver-mqtt-proof-$runStamp",
    "KAFKA_GROUP_ID=command-ack-receiver-mqtt-proof-$runStamp",
  ...
  @(
    ...
    "KAFKA_BROKERS=$kafkaBrokers",
    "KAFKA_CLIENT_ID=command-events-recorder-mqtt-proof-$runStamp",
    "KAFKA_GROUP_ID=command-events-recorder-mqtt-proof-$runStamp",
    "KAFKA_TOPIC_DEVICE_COMMAND_EVENTS=device.command_events.v1",
    "POSTGRES_HOST=$postgresHost",
    "POSTGRES_PORT=$postgresPort",
    "POSTGRES_USER=$postgresUser",
  ...
  @(
    ...
    "KAFKA_BROKERS=$kafkaBrokers",
    "KAFKA_CLIENT_ID=command-notify-worker-mqtt-proof-$runStamp",
    "KAFKA_GROUP_ID=command-notify-worker-mqtt-proof-$runStamp",
    "KAFKA_TOPIC_DEVICE_COMMAND_EVENTS=device.command_events.v1",
    "POSTGRES_HOST=$postgresHost",
    "POSTGRES_PORT=$postgresPort",
    "POSTGRES_USER=$postgresUser",
  ...
  $logDir = Join-Path $repoRoot ("backups/evidence/field-command-failed-mqtt-receipt-proof-" + $runStamp)
  ...
  try {
    $apiEnv = @{
      SERVICE_NAME = "api-service-command-failed-mqtt-receipt-proof"
      API_HOST = "0.0.0.0"
    ...
    if (-not $dashboardPassword) {
      throw "EMQX_DASHBOARD_PASSWORD is missing in infra/compose/.env"
    }
    if (-not $emqxWebhookToken) {
      throw "EMQX_WEBHOOK_TOKEN is missing for local api-service"
    }
    ...
    $deviceResp = Invoke-ApiJson "Post" ($apiBaseUrl + "/devices") @{
      deviceName = "field-command-failed-mqtt-receipt-proof-" + $runStamp
      deviceType = "multi_sensor"
      metadata = @{
        note = "field_rehearsal"
        install_label = "FIELD-CMD-FAILED-MQTT-RECEIPT"
      }
    } $headers
...
"@
    ...
    $errorMessage = "simulated mqtt receipt failure for field proof"
    Publish-MqttFailedAck $mqttUrl $deviceId $deviceSecret $commandId $errorMessage
...
"@
    ...
    if (-not $notification) {
      throw "device command failed MQTT receipt notification was not created within timeout"
    }
    ...
    $report = [ordered]@{
      ...
      conclusion = if (
        ...
        $detailResp.data.content -match "simulated mqtt receipt failure for field proof" -and
        ...
        "mqtt-command-failed-receipt-behavior-needs-review"
      }
      notes = @(
        "The proof creates a rehearsal device, issues a sent command, and publishes a failed MQTT ack to cmd_ack/<device_id>.",
        "command-ack-receiver should accept the MQTT ack, publish device.command_acks.v1, apply the failed receipt, emit COMMAND_FAILED, and keep the downstream event + notification chain readable.",
        "This proof covers cmd_ack/<device_id> -> command-ack-receiver -> device.command_acks.v1 -> device.command_events.v1 -> device_command_notifications -> API."
      )
