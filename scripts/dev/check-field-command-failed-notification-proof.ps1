...
param(
  ...
  [string]$OutFile = "docs/unified/reports/field-command-failed-notification-proof-latest.json"
)
...
function Publish-CommandFailedEvent([string]$DeviceId, [string]$CommandId, [string]$EventId, [string]$KafkaBrokers, [string]$Topic, [string]$ErrorMessage) {
  $nodeScript = @"
const { Kafka, logLevel } = require('kafkajs');

(async () => {
  const kafka = new Kafka({
    clientId: 'field-command-failed-proof',
...
try {
  ...
  $logDir = Join-Path $repoRoot ("backups/evidence/field-command-failed-proof-" + $runStamp)
  ...
  @(
    ...
    "KAFKA_CLIENT_ID=command-events-recorder-failed-proof-$runStamp",
    "KAFKA_GROUP_ID=command-events-recorder-failed-proof-$runStamp",
  ...
  @(
    ...
    "KAFKA_BROKERS=$kafkaBrokers",
    "KAFKA_CLIENT_ID=command-notify-worker-failed-proof-$runStamp",
    "KAFKA_GROUP_ID=command-notify-worker-failed-proof-$runStamp",
    "KAFKA_TOPIC_DEVICE_COMMAND_EVENTS=device.command_events.v1",
    "POSTGRES_HOST=$postgresHost",
    "POSTGRES_PORT=$postgresPort",
    "POSTGRES_USER=$postgresUser",
  ...
  $notifyProc = $null
  try {
    $env:SERVICE_NAME = "api-service-command-failed-notify-proof"
    ...
    $deviceResp = Invoke-ApiJson "Post" ($apiBaseUrl + "/devices") @{
      deviceName = "field-command-failed-proof-" + $runStamp
      deviceType = "multi_sensor"
      metadata = @{
        note = "field_rehearsal"
        install_label = "FIELD-CMD-FAILED-PROOF"
      }
    } $headers
    ...
    $errorMessage = "simulated command failed event for field proof"
    $markFailedSql = @"
...
SET
  ...
  updated_at = NOW(),
  error_message = '$errorMessage',
  result = jsonb_build_object('error_message', '$errorMessage')
WHERE command_id = '$commandId' AND device_id = '$deviceId';
"@
    $markFailedSql | docker compose -f infra/compose/docker-compose.yml --env-file infra/compose/.env exec -T postgres psql -U $postgresUser -d $postgresDatabase 1>$null 2>$null

    $eventId = [guid]::NewGuid().ToString()
    Publish-CommandFailedEvent $deviceId $commandId $eventId $kafkaBrokers $eventTopic $errorMessage
...
"@
    ...
    if (-not $notification) {
      throw "device command failed notification was not created within timeout"
    }
    ...
    $report = [ordered]@{
      ...
      conclusion = if (
        ...
        $detailResp.data.content -match "simulated command failed event for field proof" -and
        ...
        "command-failed-notification-behavior-needs-review"
      }
      notes = @(
        "The proof creates a rehearsal device, issues a command, and publishes a COMMAND_FAILED event to device.command_events.v1.",
        "command-events-recorder should persist the failed event, and command-notify-worker should create device_command_notifications.",
        "The command notification APIs should support list, stats, detail, and mark-as-read for failed command notifications."
      )
      api = [ordered]@{
        list = $listResp.data
