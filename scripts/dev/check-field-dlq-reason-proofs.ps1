...
param(
  ...
  [string]$OutFile = "docs/unified/reports/field-dlq-reason-proofs-latest.json"
)
...
function Query-DlqEntries([string]$DeviceId) {
  $prev = $ErrorActionPreference
  $ErrorActionPreference = "Continue"
  try {
    $text = docker exec lsmv2_kafka /opt/kafka/bin/kafka-console-consumer.sh --bootstrap-server kafka:9092 --topic telemetry.dlq.v1 --from-beginning --timeout-ms 3000 2>&1 | Out-String
  } finally {
...
try {
  ...
  @(
    ...
    ("KAFKA_CLIENT_ID=huawei-iot-adapter-dlq-" + $runStamp),
  ...
  @(
    ...
    "KAFKA_BROKERS=127.0.0.1:9094",
    ("KAFKA_CLIENT_ID=telemetry-writer-dlq-" + $runStamp),
    ("KAFKA_GROUP_ID=telemetry-writer-dlq-" + $runStamp),
    "KAFKA_TOPIC_TELEMETRY_RAW=telemetry.raw.v1",
  ...
  try {
    ...
    $scenarios = @(
      [pscustomobject]@{ name = "hf_oversized"; baseline = $null; followup = "hf-oversized"; expectedReason = "high_frequency_budget_exceeded" },
      [pscustomobject]@{ name = "duplicate"; baseline = "hf-normal"; followup = "hf-duplicate"; expectedReason = "duplicate_seq" },
      [pscustomobject]@{ name = "out_of_order"; baseline = "hf-normal"; followup = "hf-out-of-order"; expectedReason = "stale_seq" },
      [pscustomobject]@{ name = "replay"; baseline = "hf-normal"; followup = "hf-replay"; expectedReason = "stale_seq" }
    )
    ...
    foreach ($scenario in $scenarios) {
      ...
      $dlqEntries = Wait-ForCondition {
        $rows = Query-DlqEntries $deviceId
        if (@($rows).Count -gt 0) { return $rows }
        return $null
      } $TimeoutSeconds

      $matchedReason = @($dlqEntries | Where-Object { $_.reason_code -eq $scenario.expectedReason } | Select-Object -First 1)
      ...
      $results.Add([ordered]@{
        ...
        dlqObserved = [bool]$dlqEntries
        matchedExpectedReason = [bool]$matchedReason
        reasonCodes = @($dlqEntries | ForEach-Object { $_.reason_code })
        conclusion = if ($matchedReason) { "dlq-reason-proof-ok" } else { "dlq-reason-proof-missing" }
      }) | Out-Null
    }
    ...
    $report = [ordered]@{
      ...
      scenarios = $results
      allMatched = (@($results | Where-Object { -not $_.matchedExpectedReason }).Count -eq 0)
      notes = @(
        "This proof verifies that semantic guards not only prevent persistence, but also emit the expected telemetry.dlq.v1 reason_code.",
        "hf_oversized expects high_frequency_budget_exceeded; duplicate expects duplicate_seq; out_of_order/replay expect stale_seq."
      )
    }
