...
$tsFile = Join-Path $repoRoot "scripts/dev/check-desk-command-notify-on-ack.ts"
$outFile = Join-Path $repoRoot "docs/unified/reports/desk-command-notify-on-ack-proof-latest.json"
...
if ($LASTEXITCODE -ne 0) {
  throw "desk command notifyOnAck proof failed (exit=$LASTEXITCODE)"
}
