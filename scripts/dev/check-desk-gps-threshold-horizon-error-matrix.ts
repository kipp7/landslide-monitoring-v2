...
async function main(): Promise<void> {
  ...
  const targets = selectGpsProfileTargets(baselines.data.list, "gps threshold horizon error matrix");
    ...
    for (const horizon of horizons) {
      ...
      for (const endIndex of anchors) {
        ...
        for (const key of ["blue", "yellow", "red"] as const) {
          ...
          const predicted = (analysis.data.prediction?.shortTerm ?? []).slice(0, horizon).some((value) => value >= threshold);
  ...
  if (!creep || !event || !cyclic) {
    throw new Error("gps threshold horizon error matrix missing required profiles");
  }
  ...
  for (const horizon of ["6h", "24h", "72h"] as const) {
    if (!(creep.matrix[horizon].blue.missRate === 0 && event.matrix[horizon].blue.missRate === 0 && cyclic.matrix[horizon].blue.missRate === 0)) {
      throw new Error(`gps threshold horizon error matrix blue miss mismatch at ${horizon}`);
    }
    if (!(creep.matrix[horizon].red.missRate === 0 && event.matrix[horizon].red.missRate === 0)) {
      throw new Error(`gps threshold horizon error matrix red miss mismatch at ${horizon}`);
    }
    if (!(cyclic.matrix[horizon].yellow.falseAlarmRate === 0 && cyclic.matrix[horizon].red.falseAlarmRate === 0)) {
      throw new Error(`gps threshold horizon error matrix cyclic false alarm mismatch at ${horizon}`);
    }
  }

  const report = {
    gpsThresholdHorizonErrorMatrix: {
