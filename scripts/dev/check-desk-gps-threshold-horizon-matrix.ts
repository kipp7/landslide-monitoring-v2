...
async function main(): Promise<void> {
  ...
  for (const target of targets) {
    ...
    const byHorizon: Record<string, Record<string, ReturnType<typeof metric>>> = {};
  ...
  if (!creep || !event || !cyclic) {
    throw new Error("gps threshold horizon matrix missing required profiles");
  }
  ...
  for (const horizon of ["6h", "24h", "72h"] as const) {
    if (!(creep.matrix[horizon].blue.precision === 1 && event.matrix[horizon].blue.precision === 1 && cyclic.matrix[horizon].blue.precision === 1)) {
      throw new Error(`gps threshold horizon matrix blue precision mismatch at ${horizon}`);
    }
    if (!(creep.matrix[horizon].red.precision === 1 && event.matrix[horizon].red.precision === 1)) {
      throw new Error(`gps threshold horizon matrix red precision mismatch at ${horizon}`);
    }
    if (!(cyclic.matrix[horizon].yellow.specificity === 1 && cyclic.matrix[horizon].red.specificity === 1)) {
      throw new Error(`gps threshold horizon matrix cyclic specificity mismatch at ${horizon}`);
    }
  }

  const report = {
    gpsThresholdHorizonMatrix: {
      profileCount: entries.length,
      horizons: ["6h", "24h", "72h"],
      bluePrecisionStable: true,
      redPrecisionStable: true,
