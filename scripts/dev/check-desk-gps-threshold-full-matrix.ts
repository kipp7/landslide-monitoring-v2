...
async function main(): Promise<void> {
  ...
  const targets = selectGpsProfileTargets(baselines.data.list, "gps threshold full matrix");
  ...
  if (!creep || !event || !cyclic) {
    throw new Error("gps threshold full matrix missing required profiles");
  }
  ...
  for (const horizon of ["6h", "24h", "72h"] as const) {
    if (!(creep.matrix[horizon].blue.precision === 1 && event.matrix[horizon].blue.precision === 1 && cyclic.matrix[horizon].blue.precision === 1)) {
      throw new Error(`gps threshold full matrix blue precision mismatch at ${horizon}`);
    }
    if (!(creep.matrix[horizon].blue.missRate === 0 && event.matrix[horizon].blue.missRate === 0 && cyclic.matrix[horizon].blue.missRate === 0)) {
      throw new Error(`gps threshold full matrix blue miss mismatch at ${horizon}`);
    }
    if (!(creep.matrix[horizon].red.precision === 1 && event.matrix[horizon].red.precision === 1)) {
      throw new Error(`gps threshold full matrix red precision mismatch at ${horizon}`);
    }
    if (!(creep.matrix[horizon].red.missRate === 0 && event.matrix[horizon].red.missRate === 0)) {
      throw new Error(`gps threshold full matrix red miss mismatch at ${horizon}`);
    }
    if (!(cyclic.matrix[horizon].yellow.specificity === 1 && cyclic.matrix[horizon].yellow.falseAlarmRate === 0)) {
      throw new Error(`gps threshold full matrix cyclic yellow governance mismatch at ${horizon}`);
    }
    if (!(cyclic.matrix[horizon].red.specificity === 1 && cyclic.matrix[horizon].red.falseAlarmRate === 0)) {
      throw new Error(`gps threshold full matrix cyclic red governance mismatch at ${horizon}`);
    }
  }

  const report = {
    gpsThresholdFullMatrix: {
      profileCount: entries.length,
      horizons: ["6h", "24h", "72h"],
      thresholds: ["blue", "yellow", "red"],
      precisionStable: true,
      missStable: true,
      cyclicSpecificityStable: true,
      entries
    }
  };
