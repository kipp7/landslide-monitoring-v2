...
function Convert-HexAddressToBytes {
  ...
  $address = Convert-HexAddressToUInt16 -Value $Value
  return @(
...
function Build-Cc9dFrame {
  ...
  foreach ($b in (Convert-HexAddressToBytes -Value $SessionRemoteAddress)) {
    $session.Add($b)
  }
...
function Try-DecodeCc9dPacket {
  ...
  try {
    ...
    $source = [int]$decoded[0]
    $destination = [int]$decoded[1]
    $remoteAddress = [int]$decoded[2] + (([int]$decoded[3]) -shl 8)
    $payload = if ($decoded.Length -gt 5) { $decoded[4..($decoded.Length - 2)] } else { @() }
    $payloadText = [System.Text.Encoding]::UTF8.GetString([byte[]]$payload)

    return [ordered]@{
      ok = $true
      sourcePort = $source
      destinationPort = $destination
      remoteAddress = ("0x{0}" -f $remoteAddress.ToString("X4"))
      payloadLength = $payload.Length
      payloadHex = Convert-BytesToHex -Bytes ([byte[]]$payload)
      payloadText = $payloadText
      decodedHex = Convert-BytesToHex -Bytes $decoded
      crc = ("0x{0}" -f $receivedCrc.ToString("X2"))
    }
  } catch {
    return [ordered]@{
      ok = $false
    ...
    while ($SerialPort.BytesToRead -gt 0) {
      ...
      if ($b -eq 0x9D) {
        if ($inFrame -and $currentPacket.Count -gt 0) {
          $frames.Add((Try-DecodeCc9dPacket -PacketBytes $currentPacket.ToArray())) | Out-Null
        }
...
$payloadText = ($document | ConvertTo-Json -Depth 6)
$payloadBytes = [System.Text.Encoding]::UTF8.GetBytes($payloadText)
$frameInfo = Build-Cc9dFrame -SessionSourcePort $SourcePort -SessionDestinationPort $DestinationPort -SessionRemoteAddress $RemoteAddress -PayloadBytes $payloadBytes
...
[ordered]@{
  ...
  port = $Port
  baudRate = $BaudRate
  remoteAddress = ("0x{0}" -f (Convert-HexAddressToUInt16 -Value $RemoteAddress).ToString("X4"))
  sourcePort = $SourcePort
  destinationPort = $DestinationPort
  commandId = $document.command_id
  commandType = $document.command_type
  payloadText = $payloadText
