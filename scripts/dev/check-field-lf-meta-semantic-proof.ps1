...
param(
  ...
  [string]$OutFile = "docs/unified/reports/field-lf-meta-semantic-proof-latest.json"
)
...
SELECT
  version,
  state #>> '{metrics,tilt_x_deg}' AS tilt_x_deg,
  state #>> '{metrics,tilt_y_deg}' AS tilt_y_deg,
  state #>> '{metrics,temperature_c}' AS temperature_c,
  state #>> '{metrics,humidity_pct}' AS humidity_pct,
  state #>> '{metrics,battery_pct}' AS battery_pct,
  state #>> '{meta,fw}' AS fw,
  state #>> '{meta,sampling_s}' AS sampling_s,
  state #>> '{meta,report_interval_s}' AS report_interval_s
FROM device_state
...
"@
  ...
  return [pscustomobject]@{
    ...
    humidity_pct = [double]$parts[4]
    battery_pct = [double]$parts[5]
    fw = [string]$parts[6]
    sampling_s = [int]$parts[7]
    report_interval_s = [int]$parts[8]
  }
}
...
try {
  ...
  @(
    ...
    ("KAFKA_CLIENT_ID=huawei-iot-adapter-lfmeta-" + $runStamp),
  ...
  @(
    ...
    "KAFKA_BROKERS=127.0.0.1:9094",
    ("KAFKA_CLIENT_ID=telemetry-writer-lfmeta-" + $runStamp),
    ("KAFKA_GROUP_ID=telemetry-writer-lfmeta-" + $runStamp),
    "KAFKA_TOPIC_TELEMETRY_RAW=telemetry.raw.v1",
  ...
  try {
    ...
    Publish-Sample $baseUrl "lf-meta" $deviceId | Out-Null
    ...
    $report = [ordered]@{
      ...
      conclusion = if (
        $state -and
        [Math]::Abs([double]$state.tilt_x_deg - 0.18) -lt 0.0001 -and
        [Math]::Abs([double]$state.tilt_y_deg - 0.09) -lt 0.0001 -and
        [Math]::Abs([double]$state.temperature_c - 25.3) -lt 0.0001 -and
        [Math]::Abs([double]$state.humidity_pct - 62.1) -lt 0.0001 -and
        $state.fw -eq "rk2206-sim-0.1.0" -and
        $state.sampling_s -eq 5 -and
        $state.report_interval_s -eq 30
      ) {
        "lf-meta-merges-without-overwriting-high-frequency-state"
      } else {
        "lf-meta-behavior-needs-review"
      }
      notes = @(
        "hf-normal establishes the latest high-frequency tilt metrics.",
        "lf-meta should enrich state.meta and low-frequency metrics without deleting prior high-frequency tilt metrics."
      )
    }
