...
async function main(): Promise<void> {
  ...
  const targets = selectGpsProfileTargets(baselines.data.list, "gps threshold ranking");
  ...
  if (!first || !second || !third) {
    throw new Error("gps threshold ranking incomplete");
  }
  if (!(first.profile === "event_acceleration" && second.profile === "creep_rise" && third.profile === "cyclic_oscillation")) {
    throw new Error("gps threshold ranking order mismatch");
  }
