[CmdletBinding()]
param(
  [ValidateSet("handshake", "read-config", "read-version", "led")]
...
$destinationPort = 102
$payloadBytes = switch ($Action) {
  "handshake" { [byte[]](0x48) }
  "read-config" { $destinationPort = 103; [byte[]](0x52) }
  "read-version" { [byte[]](0x56) }
  "led" { [byte[]](0x4C) }
  default { throw "Unsupported action: $Action" }
}

$frameInfo = Build-Cc9dFrame -SessionSourcePort $SourcePort -SessionDestinationPort $destinationPort -SessionRemoteAddress 0xFFF0 -PayloadBytes $payloadBytes
