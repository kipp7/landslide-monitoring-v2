...
param(
  ...
  [string]$OutFile = "docs/unified/reports/field-command-success-notification-policy-custom-type-proof-latest.json"
)
...
(async () => {
  const kafka = new Kafka({
    clientId: 'field-command-success-policy-custom-type-proof',
    ...
    messages: [
      {
        ...
        value: JSON.stringify({
          ...
          result: {
            applied: true,
            source: 'command_success_notification_policy_custom_type_proof'
          }
        })
      }
    ]
...
try {
  ...
  $customCommandType = "custom_success_policy_proof"

  $notifyEnvPath = Join-Path $repoRoot "services/command-notify-worker/.env"
  $logDir = Join-Path $repoRoot ("backups/evidence/field-command-success-notification-policy-custom-type-proof-" + $runStamp)
  ...
  @(
    ...
    "KAFKA_CLIENT_ID=command-notify-worker-success-policy-custom-type-proof-$runStamp",
    "KAFKA_GROUP_ID=command-notify-worker-success-policy-custom-type-proof-$runStamp",
  ...
  $originalPolicy = $null
  try {
    $env:SERVICE_NAME = "api-service-command-success-policy-custom-type-proof"
    ...
    $updatedDefaults[$customCommandType] = "always_notify"

    $updatedPolicy = @{
      systemDefault = [string]$originalPolicy.systemDefault
    ...
    $deviceResp = Invoke-ApiJson "Post" ($apiBaseUrl + "/devices") @{
      deviceName = "field-command-success-policy-custom-type-proof-" + $runStamp
      deviceType = "multi_sensor"
      metadata = @{
        note = "field_rehearsal"
        install_label = "FIELD-CMD-SUCCESS-POLICY-CUSTOM-TYPE"
      }
    } $headers
    ...
    $commandResp = Invoke-ApiJson "Post" ($apiBaseUrl + "/devices/$deviceId/commands") @{
      ...
      payload = @{
        source = "field-command-success-policy-custom-type-proof"
        expectedPolicy = "custom_type_added_via_management_api"
      }
    } $headers
...
SET
  ...
  acked_at = NOW(),
  updated_at = NOW(),
  result = '{"applied": true, "source": "command_success_notification_policy_custom_type_proof"}'::jsonb,
  error_message = NULL
...
INSERT INTO device_command_events (
  ...
  '$deviceId'::uuid,
  'acked',
  'command success notification policy custom type proof',
  '{"applied": true, "source": "command_success_notification_policy_custom_type_proof"}'::jsonb,
  NOW()
)
...
"@
    ...
    if (-not $notification) {
      throw "custom command type proof did not create a notification within timeout"
    }
    ...
    $report = [ordered]@{
      ...
      conclusion = if (
        [string]$commandStateResp.data.commandType -eq $customCommandType -and
        ...
        @($notificationListResp.data.list).Count -ge 1 -and
        [int]$notificationStatsResp.data.totals.total -ge 1 -and
        [int]$notificationStatsResp.data.totals.unread -ge 1
      ) {
        "command-success-notification-policy-custom-type-now-controls-runtime-behavior"
      } else {
        "command-success-notification-policy-custom-type-behavior-needs-review"
      }
      notes = @(
        "The proof reads the current command success-notification policy through the dedicated system API, adds a new custom command type default through the same API, and then issues that custom command type.",
        "A custom command type added to commandTypeDefaults should be treated as a first-class default table entry and resolve inherit to always_notify.",
        "command-notify-worker should therefore create a COMMAND_ACKED notification for the new custom command type."
      )
