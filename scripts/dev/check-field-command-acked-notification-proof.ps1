...
param(
  ...
  [string]$OutFile = "docs/unified/reports/field-command-acked-notification-proof-latest.json"
)
...
(async () => {
  const kafka = new Kafka({
    clientId: 'field-command-acked-proof',
...
try {
  ...
  $logDir = Join-Path $repoRoot ("backups/evidence/field-command-acked-notification-proof-" + $runStamp)
  ...
  @(
    ...
    "KAFKA_CLIENT_ID=command-notify-worker-acked-notify-proof-$runStamp",
    "KAFKA_GROUP_ID=command-notify-worker-acked-notify-proof-$runStamp",
  ...
  $notifyProc = $null
  try {
    $env:SERVICE_NAME = "api-service-command-acked-notification-proof"
...
"@
    ...
    $deviceResp = Invoke-ApiJson "Post" ($apiBaseUrl + "/devices") @{
      deviceName = "field-command-acked-notification-proof-" + $runStamp
      deviceType = "multi_sensor"
      metadata = @{
        note = "field_rehearsal"
        install_label = "FIELD-CMD-ACKED-NOTIFY"
      }
    } $headers
...
"@
    ...
    if (-not $notification) {
      throw "device command acked notification was not created within timeout"
    }
    ...
    $report = [ordered]@{
      ...
      conclusion = if (
        ...
        "command-acked-notification-behavior-needs-review"
      }
      notes = @(
        "The proof creates a rehearsal device, issues a command with notifyOnAck=true, inserts the matching device_command_events row, and publishes a COMMAND_ACKED event to device.command_events.v1.",
        "command-notify-worker should create device_command_notifications only because notifyOnAck is enabled on the command.",
        "The command notification APIs should support list, stats, detail, and mark-as-read for opt-in acked notifications."
      )
