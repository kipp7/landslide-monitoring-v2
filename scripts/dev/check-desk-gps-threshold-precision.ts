...
async function main(): Promise<void> {
  ...
  const targets = selectGpsProfileTargets(baselines.data.list, "gps threshold precision");
  ...
  if (!creep || !event || !cyclic) {
    throw new Error("gps threshold precision missing required profiles");
  }

  if (!(creep.blue.precision === 1 && event.blue.precision === 1 && cyclic.blue.precision === 1)) {
    throw new Error("gps threshold precision blue precision mismatch");
  }
  if (!(creep.yellow.precision === 1 && event.yellow.precision === 1)) {
    throw new Error("gps threshold precision yellow precision mismatch");
  }
  if (!(creep.red.precision === 1 && event.red.precision === 1)) {
    throw new Error("gps threshold precision red precision mismatch");
  }
  if (!(cyclic.yellow.specificity === 1 && cyclic.red.specificity === 1)) {
    throw new Error("gps threshold precision cyclic specificity mismatch");
  }

  const report = {
    gpsThresholdPrecision: {
      profileCount: entries.length,
      bluePrecisionStable: true,
      yellowPrecisionStable: true,
