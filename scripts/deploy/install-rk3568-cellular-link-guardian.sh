...
{"generatedAt":"${now}","ok":${ok},"reason":"${reason}","cloudHost":"${CLOUD_HOST}","usbDevice":"${USB_DEVICE}","usbGateway":"${USB_GATEWAY}","routeDevice":"${route_device}","detail":"${detail_json}"}
JSON
}
...
route_cloud_via_usb() {
  ip route replace "${CLOUD_HOST}/32" via "${USB_GATEWAY}" dev "${USB_DEVICE}" metric 20
...
route_cloud_via_lan() {
  ip route replace "${CLOUD_HOST}/32" via "${LAN_GATEWAY}" dev "${LAN_DEVICE}" metric 10
}
...
route_cloud_via_usb
usb_route=$(ip route get "${CLOUD_HOST}" 2>&1 || true)
...
if [[ "${ALLOW_LAN_FALLBACK}" == "1" ]] && ip link show "${LAN_DEVICE}" >/dev/null 2>&1; then
  route_cloud_via_lan
  lan_route=$(ip route get "${CLOUD_HOST}" 2>&1 || true)
