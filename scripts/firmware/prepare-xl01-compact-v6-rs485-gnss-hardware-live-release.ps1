...
param(
  ...
  [string]$ReleaseDirectory = "F:\2\openharmony\rk2206_firmware_releases\xls1_compact_v6_protected_p1_rs485_gnss_hardware_live_20260804",
...
$stagingRoot = Join-Path ([System.IO.Path]::GetTempPath()) `
  ("rk2206-compact-v6-rs485-gnss-hardware-live-" + [guid]::NewGuid().ToString("N"))
...
try {
  ...
  $instructions = @"
Compact V6 protected-P1 hardware GNSS + LIVE-capable release
...
"@
  ...
  Write-Host "COMPACT_V6_HARDWARE_GNSS_RELEASE_OK path=$releaseRoot manifest_sha256=$manifestSha256 source=$headCommit"
} finally {
