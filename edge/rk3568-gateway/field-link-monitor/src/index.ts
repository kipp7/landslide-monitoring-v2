...
type AutomationPlanReport = {
  ...
  governance: {
    supervisionBoundary: "consume-read-only-plan";
...
class FieldLinkMonitor {
  ...
  private buildSummary(gatewayResult: ReadJsonResult, networkResult: ReadJsonResult): LinkSummaryReport {
    ...
    const summaryReport: Omit<LinkSummaryReport, "automation"> = {
      ...
      recommendations: [
        "consume this summary from local UI or supervision layers only in read-only mode",
      ...
      if (dimension && levelRank(dimension.level) >= levelRank("attention")) {
        tasks.push(
          newAutomationTask(
            ...
            dimension.summary,
            dimension.evidence,
            "检查对应 systemd 服务是否仍在写入状态文件；监督层只读取该计划，不直接替代源服务写入。",
            "operator_required"
          )
        );
      }
    ...
    if (network && network.level !== "healthy") {
      tasks.push(
        newAutomationTask(
          ...
          network.summary,
          network.evidence,
          "若处于 AP fallback，优先现场确认 STA 回传、路由器和供电；不要由监督层自动切 Wi-Fi。",
          "operator_required"
        )
      );
    }
    ...
    if (tasks.length === 0) {
      tasks.push(
        newAutomationTask(
          ...
          "保持 field-link-monitor 只读巡检；本地 UI 或监督层可展示状态，但无需执行动作。",
          "read_only"
        )
      );
    }
