...
SYSTEMD_UNIT_NAME="${SYSTEMD_UNIT_NAME:-lsmv2-field-link-monitor}"
ENV_FILE_PATH="${ENV_FILE_PATH:-/etc/lsmv2/field-link-monitor.env}"
STATE_ROOT="${STATE_ROOT:-/var/lib/lsmv2/field-link-monitor}"
GATEWAY_HEALTH_FILE_PATH="${GATEWAY_HEALTH_FILE_PATH:-/var/lib/lsmv2/field-gateway/health/runtime-health.json}"
NETWORK_STATUS_FILE_PATH="${NETWORK_STATUS_FILE_PATH:-/var/lib/lsmv2/network-bootstrap/status/runtime-status.json}"
...
usage() {
  cat <<'EOF'
Usage:
  sudo bash edge/rk3568-gateway/field-link-monitor/deploy/install-rk3568-field-link-monitor.sh [options]
...
Options:
  ...
  --gateway-health-file <path>   field-gateway runtime health file path
  --network-status-file <path>   network bootstrap runtime status file path
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --gateway-health-file) GATEWAY_HEALTH_FILE_PATH="$2"; shift 2 ;;
    --network-status-file) NETWORK_STATUS_FILE_PATH="$2"; shift 2 ;;
...
if [[ ! -d "${REPO_ROOT}/edge/rk3568-gateway/field-link-monitor" ]]; then
  echo "Service directory not found under repo root: ${REPO_ROOT}/edge/rk3568-gateway/field-link-monitor" >&2
  exit 1
fi
...
if [[ "${BUILD_FIRST}" -eq 1 ]]; then
  ...
  runuser -u "${RUN_USER}" -- env PATH="${PATH}" bash -lc "
    cd \"${REPO_ROOT}\"
    rm -rf \
      \"${REPO_ROOT}/edge/rk3568-gateway/field-link-monitor/dist\" \
      \"${REPO_ROOT}/packages/observability/dist\"
    npm install
    npm run build \
      --workspace @lsmv2/observability \
      --workspace @lsmv2/field-link-monitor
  "
fi
...
SERVICE_NAME=field-link-monitor
GATEWAY_HEALTH_FILE_PATH=${GATEWAY_HEALTH_FILE_PATH}
NETWORK_STATUS_FILE_PATH=${NETWORK_STATUS_FILE_PATH}
SUMMARY_FILE_PATH=${STATE_ROOT}/status/summary.json
HTTP_HOST=${HTTP_HOST}
HTTP_PORT=${HTTP_PORT}
POLL_INTERVAL_MS=5000
PUBLISH_FRESHNESS_MS=30000
...
sed \
  ...
  -e "s|__WORKING_DIRECTORY__|${REPO_ROOT}/edge/rk3568-gateway/field-link-monitor|g" \
  -e "s|__ENV_FILE_PATH__|${ENV_FILE_PATH}|g" \
  -e "s|__STATE_ROOT__|${STATE_ROOT}|g" \
  "${SCRIPT_DIR}/field-link-monitor.service.template" > "${SYSTEMD_UNIT_PATH}"
...
echo "Working directory: ${REPO_ROOT}/edge/rk3568-gateway/field-link-monitor"
