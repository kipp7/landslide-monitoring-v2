...
const configSchema = z
  .object({
    ...
    southboundCorePollDeadlineMs: z.coerce.number().int().min(1000).max(10000).default(2000),
    southboundLayeredEnvironmentEveryRounds: z.coerce.number().int().min(1).max(1000).default(3),
    southboundLayeredAuditEveryRounds: z.coerce.number().int().min(2).max(10000).default(5),
    ...
    rtcmSessionRefreshMs: z.coerce.number().int().min(5000).max(120000).default(10000),
    ...
    rtcmMaxFragmentsPerFieldFrame: z.coerce.number().int().min(1).max(4).default(1),
    rtcmMaxFragmentsBetweenPolls: z.coerce.number().int().min(1).max(32).default(4),
    rtcmPostBurstPollGuardMs: z.coerce.number().int().min(100).max(2000).default(600),
    rtcmMinCorrectionWindowMs: z.coerce.number().int().min(0).max(5000).default(0),
...
export function loadConfigFromEnv(env: NodeJS.ProcessEnv): AppConfig {
  return configSchema.parse({
    ...
    southboundCorePollDeadlineMs: env.SOUTHBOUND_CORE_POLL_DEADLINE_MS,
    ...
    rtcmMaxFragmentsPerFieldFrame: env.RTCM_MAX_FRAGMENTS_PER_FIELD_FRAME,
    rtcmMaxFragmentsBetweenPolls: env.RTCM_MAX_FRAGMENTS_BETWEEN_POLLS,
    rtcmPostBurstPollGuardMs: env.RTCM_POST_BURST_POLL_GUARD_MS,
    rtcmMinCorrectionWindowMs: env.RTCM_MIN_CORRECTION_WINDOW_MS,
