...
- `SOUTHBOUND_POLLING_PARTIAL_RETRIES` - `0` or `1`; `1` enables one bounded P2 recovery pass over nodes missing from a compact broadcast round. Targeted-only polling remains retry-free.
- `SOUTHBOUND_POLLING_RETRY_AFTER_MS` - initial P1 response window and each targeted recovery window; the V6 hybrid profile uses `1500` ms.
...
- `SOUTHBOUND_LAYERED_ENVIRONMENT_EVERY_ROUNDS` / `SOUTHBOUND_LAYERED_AUDIT_EVERY_ROUNDS` - V6 P3/P4 extension cadence; the measured hybrid profile is `30` and `60` completed core rounds.
