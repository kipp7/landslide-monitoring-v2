...
const AIPredictionComponent = () => {
  ...
  const sensorData = useMemo(() => data.slice().reverse(), [data])
  ...
  return (
    <div className="h-full flex flex-col items-center justify-center p-4 bg-[#112c42] rounded-lg max-w-[600px] mx-auto relative overflow-hidden">
      <div className="absolute top-[-50px] left-[-50px] w-32 h-32 bg-cyan-500 rounded-full opacity-20" />
      <div className="absolute bottom-[-50px] right-[-50px] w-32 h-32 bg-cyan-500 rounded-full opacity-20" />
      <div className="absolute top-[20%] left-[30%] w-16 h-16 bg-cyan-500 rounded-full opacity-20" />
      <div className="absolute bottom-[20%] right-[30%] w-16 h-16 bg-cyan-500 rounded-full opacity-20" />
      ...
      {loading ? (
        ...
        <div className="text-white text-center w-full">
          <div className="mb-4 text-center">
            <h3 className="text-lg font-semibold text-cyan-400 flex items-center justify-center">AI智能分析报告</h3>
            <div className="text-xs text-gray-400 mt-1">{data && data.length > 0 ? `基于 ${data.length} 条实时数据` : '基于系统状态'}</div>
          </div>
          ...
          <p className="text-xs text-gray-400 absolute bottom-0 right-0 mr-2.5 mb-84.5">{prediction.timestamp}</p>
          <button
        ...
        <div className="text-center">
          ...
          <div className="mt-2 text-xs text-gray-400">
            {data && data.length > 0 ? `基于 ${data.length} 条实时数据进行分析` : '基于系统状态进行分析'}
          </div>
        </div>
      )}
    </div>
