...
export default function TemperatureChart() {
  ...
  if (loading) return <div className="text-sm text-white">加载中...</div>
  if (error) return <div className="text-sm text-red-500">加载失败: {error.message}</div>
  if (isEmpty) return <div className="text-sm text-white">暂无数据</div>
  ...
  const calculateLegendSpace = () => {
    ...
    const legendSpaceNeeded = deviceCount * (maxNameLength * 6 + 40)
    const containerWidth = 300
    const needsWrapping = legendSpaceNeeded > containerWidth
    return needsWrapping ? '25%' : '20%'
  }
