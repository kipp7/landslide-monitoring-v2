...
function normalizeClusters(clusters: Cluster[], zoom: number, selectedDeviceId?: string): Cluster[] {
  const grid = gridSizeByZoom(zoom)
  if (!grid) return clusters

  const buckets = new Map<string, { latSum: number; lonSum: number; stations: Cluster['stations'] }>()
        ...
        <Popup>
          <div className="space-y-2">
            <div className="space-y-0.5">
              ...
              {grid ? <div className="text-xs text-slate-600">Tip: click marker to zoom in.</div> : null}
            </div>
...
export default function StationMap({
  ...
  const clusters = useMemo(() => {
    const byStationId = new Map<string, Cluster['stations'][number]>()

    for (const p of points) {
      const existing = byStationId.get(p.stationId)
      if (existing) {
        existing.devices.push(p)
      } else {
        byStationId.set(p.stationId, {
    ...
    const clusters = Array.from(byStationId.values()).map((s): Cluster => {
      const devices = s.devices.slice().sort((a, b) => {
    ...
    return normalizeClusters(clusters, zoom, selectedDeviceId)
