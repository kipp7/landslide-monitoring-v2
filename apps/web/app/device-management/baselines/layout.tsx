...
export default function BaselinesLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!tokenPresent) {
    return (
      <Card>
        <Text type="secondary">
          需要配置 Bearer Token 或登录后访问：<Link href="/login">去登录</Link>（或在 <Link href="/settings">设置</Link> 中配置手动 token）
        </Text>
      </Card>
    )
  }
