...
export default function DeviceManagementPage() {
  ...
  const fetchRealTimeData = useCallback(async (showMessage = false) => {
    try {
      ...
      // 使用真实的数据库设备API获取特定设备数据
      const response = await fetch(`/api/device-management-real-db?device_id=${selectedDevice}&mode=device_specific`);
  ...
  const handleDeviceDiagnostics = useCallback(async () => {
    try {
      ...
      // 调用真实的设备诊断API
      const response = await fetch(`/api/device-management-real/diagnostics`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          device_id: device.actual_device_id || selectedDevice,
          simple_id: selectedDevice
        })
      });
