...
山体滑坡监测 V2 是一个公开的端到端监测系统工作区。仓库把当前维护的系统模块放在一起，同时把生成产物、私有部署配置和完整供应商 SDK 排除在 Git 之外。
...
公开树包含源码、文档、示例、部署模板和经过筛选的硬件交付资料。生产后端服务、客户特定部署配置、本地现场证据和发布产物不放入公开仓库。
...
- `hermes-edge-supervisor` 将本地证据转为安全的诊断和操作建议。
...
RK2206 XL01 固件包包含项目自有应用代码、配置、驱动、引脚说明和构建元数据。它不是完整 OpenHarmony vendor SDK。构建时应把该包放入预期 vendor 树，并使用 OpenHarmony/RK2206 构建流程。
...
- 公开模块必须对应清晰的产品职责。
- 使用可读目录名，避免历史内部缩写。
- 生成产物、本地报告和机器状态不提交到 Git。
- 通过 mock 数据支持无后端 UI 开发。
- RK3568 服务保持可独立构建和 lint。
- 硬件资料作为可复核交付资产维护，不作为未经检查的打板保证。
