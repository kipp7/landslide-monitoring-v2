...
- 兼容参考区 Next API 的 fallback 行为：当 v2 的上游 legacy 端点不可用时，`/devices/{deviceId}` 对 `device_1~device_3` 返回示例设备数据；`/devices/info/{simpleId}` 仅对 `device_1` 返回示例数据，其余返回 404，避免旧页面/调试页直接报错。
