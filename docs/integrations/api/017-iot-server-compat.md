...
- 兼容参考区 Next API 的 fallback 行为：当 v2 的上游 legacy 端点不可用时，`/devices/mappings`、`/devices/list`、`/devices/{deviceId}`、`/devices/info/{simpleId}` 会返回 `200` + `is_fallback: true` 的结果（列表为空或返回占位设备对象），避免旧页面直接报错。
