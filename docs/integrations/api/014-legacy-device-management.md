...
- `GET /api/monitoring-stations`：返回监测站（设备）列表；会从 `devices.metadata` 回填 `station_name`/`location_name`/`risk_level`/`sensor_types`/`chart_legend_name`/`status` 等字段；支持 `chartType` 参数返回图表配置（兼容旧页面图表；与 `GET /api/monitoring-stations/chart-config` 对齐）
