...
板端屏幕只显示终端和服务端提供的风险信息，不伪造传感器数值。待机页不得同时使用“设备正常”和“准备告警”作为并列状态；没有告警上下文时改为显示设备 ID、固件版本、revision、设备状态，以及独立的 Wi-Fi/MQTT 中文状态。蜂鸣器保持套件原始 `2 kHz / 50%` 参数，100 ms 起音错峰仅用于减轻马达启动电流压低音头；共同工作阶段仍明显变小时，应优先检查 USB 电源和线缆压降。
