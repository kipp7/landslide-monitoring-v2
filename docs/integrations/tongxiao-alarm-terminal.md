...
| --- | --- | --- | --- | --- | --- |
| `idle` | `normal` | 关闭 | 关闭 | 设备正常、风险等级、设备与联网状态 | 可选单次 `ALL_CLEAR_01` |
| `active` | `high` | 蜂鸣器先起音 100 ms，随后与马达共同间歇启动 | 红色闪烁 | 高风险、标题、建议、监测点、告警编号 | `PREPARE_01` |
| `active` | `critical` | 蜂鸣器先起音 100 ms，随后与马达共同快速间歇启动 | 红色快速闪烁 | 严重风险、立即撤离、告警上下文 | `EVACUATE_01`，之后可用 `EVACUATE_REPEAT_01` |
