...
| --- | --- | --- | --- | --- | --- | --- |
| `idle` | `normal` | 关闭 | 关闭 | 熄灭 | 设备正常、风险等级、设备与联网状态 | `ALL_CLEAR_01` 间隔 12 秒共 3 次 |
| `active` | `high` | 蜂鸣器先起音 100 ms，随后与马达共同间歇启动 | 红色闪烁 | 同步闪烁 | 高风险、标题、建议、监测点、告警编号 | `PREPARE_01` 每 30 秒重复至消音/解除 |
| `active` | `critical` | 蜂鸣器先起音 100 ms，随后与马达共同快速间歇启动 | 红色快速闪烁 | 同步快速闪烁 | 严重风险、立即撤离、告警上下文 | 首次 `EVACUATE_01`，25 秒后每 15 秒 `EVACUATE_REPEAT_01` 至消音/解除 |
...
告警页“监测”显示产生风险事件的 `station_id`，“告警”显示该事件的 `alert_id`。`station_id` 用于确定哪一个固定监测点触发；同一告警的触发、升级、确认和解除共享一个 `alert_id`。两者都是追踪标识而非传感器数值，板端对超过 20 个字符的 ID 显示前 8 位、`...` 和后 8 位。
...
语音试验镜像只对非 retained 的新 revision 播报。上电、Wi-Fi/MQTT 重连和 retained 状态恢复不得播报；本地消音与服务端静音会停止重复播报。高风险每 30 秒重复完整准备撤离词；严重告警首次完整播报后等待 25 秒，再每 15 秒重复精简撤离词；解除通知间隔 12 秒共播放 3 次。生产桥接器还必须显式配置 `TONGXIAO_VOICE_ENABLED=true` 才会在 desired state 中下发固定 phrase ID。
...
- `/silence` 只静音并保留风险信息；`/alarm_off` 才回到正常状态，并允许解除通知按 12 秒间隔播放 3 次。
