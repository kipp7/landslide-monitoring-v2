...
## 4) Web `notifyOnAck` 最小回归

如果你只想确认 Web API module 已经能消费命令成功通知策略字段：

- `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/dev/check-web-command-notify-on-ack.ps1`
...
- 优先探测本地 `http://127.0.0.1:8081/health`
- 若本地 API 未启动，则自动执行 `scripts/dev/restart-local-api-service.ps1 -SkipBuild`
- 使用 Web API module 创建一条默认静默命令与一条 `notifyOnAck=true` 命令
- 再从命令列表读回并断言：
  - `notifyOnAck`
