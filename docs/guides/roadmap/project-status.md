...
- 2025-12-27：fix（Web Next API proxy 支持 `BACKEND_URL` fallback）：`apps/web/app/api/_proxy.ts` 的 baseUrl 增加对 `BACKEND_URL`（以及 `NEXT_PUBLIC_BACKEND_URL`）的 fallback，兼容参考区部署环境只配置 BACKEND_URL 的场景（PR TBD）。
