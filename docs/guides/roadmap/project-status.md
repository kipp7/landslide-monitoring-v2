...
最后更新时间：2026-01-05（PR #309：Desk 首页卡片水平对齐 + 更新桌面端/网页品牌图标；2025-12-28：fix：Stage5Regression infra 凭据对齐；fix：Stage5Regression ClickHouse init 兼容历史环境；fix：Stage5Regression API 启动冲突修复；PR #294：API：camera legacy compat 补齐 `PUT/DELETE /api/camera`（含 `/iot/api/camera`），并更新 `docs/integrations/api/011-camera.md`；PR #289：Web：`/debug-api` 恢复参考区同款调试页（不再重定向到 `/ops/debug-api`）；PR #283：WS-K.33：monitoring-stations 图表配置兼容；兼容 legacy `/api/monitoring-stations?chartType=...` 返回配置字段，并补齐 `/api/monitoring-stations/chart-config` 路由代理；2025-12-27：PR #288：Web：HoverSidebar 默认选中项行为对齐参考区；PR #287：Web：补齐 Next API proxy：`/api/monitoring-stations/chart-config` + `/api/monitoring-stations/chart-legends`；PR #280：Web：补齐 `apps/web/package-lock.json`（对齐参考区；修复 apps/web Dockerfile `npm ci`）；PR #281：WS-K.30：iot-server-compat `/iot/*` 前缀别名；PR #274：WS-K.31：monitoring-stations chart config endpoints；PR #278：WS-K.32：monitoring-stations 列表/详情字段从 devices.metadata 回读 + 在线状态字段补齐；PR #272：WS-G.6：ops debug 兼容 legacy debug endpoints smoke tests；PR #271：WS-G.5：safe db-admin（只读 + feature gate）；PR #269：WS-G.4：恢复 legacy debug endpoints（安全只读）；PR #277：WS-O.6：补齐参考区遗留文件 `frontend/app/device-management/page.tsx.backup`；PR #273：WS-K.28：legacy `/api/monitoring-stations/{deviceId}` `PUT`/`DELETE` 补齐；PR #270：Web `/login` 登录页 UI 1:1 对齐参考区；PR #268：Web `/` 入口重定向对齐参考区（`/` → `/login`）；PR #267：/analysis legacy 1:1 对齐 Shadow error 行为；PR #265：docs/api：扩充 iot-server /info endpoints 索引；WS-K.23：legacy frontend/lib 入口补齐（apps/web/lib/{config,supabaseClient,useIotDataStore}），PR #266；WS-K.22：legacy app/api route proxies（Next route handlers → api-service `/api/*`），PR #264；WS-K.21：legacy /gps-monitoring 1:1 + /api/gps-deformation CEEMD/预测补齐，PR #263；fix：iot-server /devices/:deviceId/management + /status 兼容，PR #262；fix：iot-server /health + /info shape 对齐，PR #261；fix：/health 返回结构对齐；docs：对齐 roadmap/workstreams 状态；WS-K.20：/iot/health + /iot/huawei/* 兼容，PR #258；WS-N.22：参考区 app/utils 1:1 入口补齐，PR #257；WS-K.19：legacy data-aggregation clear_cache，PR #256；WS-K.18：iot-server /devices 兼容，PR #254；WS-N.21：参考区 app/hooks 1:1 入口补齐，PR #255；WS-K.17：legacy camera alias，PR #252；WS-K.16：legacy baselines alias routes，PR #251；WS-K.15：legacy compat aliases 补齐，PR #247；WS-N.20：/analysis legacy 占位组件还原）
- 2025-12-30：新增 Windows 桌面端原型：`apps/desk`（Mock UI 原型）+ `apps/desk-win`（WPF + WebView2 壳，支持加载 dev server 或 `apps/desk/dist` 静态资源）。
...
- 2025-12-28：Web：提升 Windows 下 `next build` 稳定性（默认补齐 `NODE_OPTIONS=--max-old-space-size=8192`，并用 `cmd.exe` 执行 `next build`，降低 OOM / `spawn EINVAL` 失败概率）。
...
- 2025-12-27：PR #280，Web：补齐 `apps/web/package-lock.json`（对齐参考区；修复 apps/web Dockerfile `npm ci`）。
- 2025-12-27：PR #281，WS-K.30：为 iot-server-compat 增加 `/iot/*` 前缀别名（兼容 baseUrl=/iot：`/iot/info`、`/iot/devices/*`、`/iot/debug/latest-data*`），并补齐文档说明。
- 2025-12-27：PR #274，WS-K.31：补齐 legacy `monitoring-stations` 图表相关端点：`/api/monitoring-stations/chart-config`、`/api/monitoring-stations/chart-legends`；并让 `/api/monitoring-stations?chartType=...` 返回与 `/chart-config` 一致（含 `deviceLegends`）。
- 2025-12-27：PR #278，WS-K.32：monitoring-stations 列表/详情字段从 devices.metadata 回读（station_name/location_name/risk_level/sensor_types/chart_legend_name/status 等），并基于 devices.last_seen_at 补齐 online_status/is_online/last_data_time，以保持旧前端配置刷新后不丢。
- 2025-12-27：PR #272，WS-G.6：`/ops/debug-api` 补齐 legacy debug endpoints smoke tests（`/api/test-db`、`/api/inspect-*`、`/api/test-expert-health`）；并对 `POST /api/db-admin` 增加输入 `DBADMIN` 的二次确认（若端点禁用/无权限则允许 403）。
- 2025-12-27：PR #271，WS-G.5：实现 legacy `/api/db-admin` 的安全只读版本（query/analyze/backup）；默认关闭（`DB_ADMIN_ENABLED=false`），并加 auth + RBAC；禁止 insert/update/delete 等写操作。
- 2025-12-27：PR #269，WS-G.4：恢复参考区 debug endpoints（`/api/test-db`、`/api/inspect-*`、`/api/test-expert-health`）；以 Fastify safe read-only routes 实现，并用 admin token/JWT + permission gate 保护。
- 2025-12-27：PR #277，WS-O.6：补齐参考区遗留文件 `frontend/app/device-management/page.tsx.backup`（仅文件不缺失；不参与运行时路由）。
...
- 2025-12-27：PR #265，docs/api：扩充 iot-server compat 的 `/info.endpoints` key，使其更接近参考区 iot-server 的 endpoints 列表（仅自描述索引，不改变 API 行为）。
- 2025-12-26：WS-K.23（legacy frontend/lib 入口补齐）：PR #266，补齐参考区 `frontend/lib/{config,supabaseClient,useIotDataStore}.ts` 在 v2 的落点（供旧组件/页面引用路径兼容）；其中 `supabaseClient` 在 v2 显式 stub（不直连 Supabase），`useIotDataStore` 改为基于 legacy `/api/iot/devices/mappings` + `/api/device-management?data_only=true` 拉取数据，并提供最小 SSE 刷新逻辑。
- 2025-12-26：WS-K.22（legacy app/api route proxies）：PR #264，补齐参考区 `frontend/app/api/**/route.ts` 在 v2 Web 的落点（以 Next route handlers 代理到 v2 api-service legacy compat `/api/*`）；同时保留参考区遗留的备份/测试页面文件（`page-old/test/page_backup`），并在根包 devDependencies 明确 `ajv@8`，避免 workspace lint/build 意外落到 `ajv@6`（缺少 `ajv/dist/2020`）。
...
- 2025-12-26：fix（iot-server /health + /info shape 对齐）：PR #261，补齐 `/health`（与 `/iot/health`）的 `status/port` 字段，并扩充 `/info` 的 `name/version/description` 字段，兼容参考区探针/脚本的期望字段（保留现有字段不破坏 v2）。
...
- 2025-12-27：WS-K.31（PR #274）：补齐 legacy `monitoring-stations` 图表相关兼容入口（`/api/monitoring-stations/chart-config`、`/api/monitoring-stations/chart-legends`）。
