...
- 2025-12-26：fix（iot-server /iot 前缀别名兼容）：为 iot-server compat 路由额外挂载 `/iot/*` 前缀（如 `GET /iot/info`、`GET /iot/devices/mappings`、`GET /iot/debug/latest-data`），兼容参考区前端默认 baseUrl=/iot 的调用方式（PR TBD）。
