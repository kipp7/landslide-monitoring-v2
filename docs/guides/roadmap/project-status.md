...
- 2025-12-26：fix（iot-server /devices/:deviceId/management + /status 兼容）：PR #262，补齐参考区 iot-server 的设备管理聚合端点与设备状态端点（内部转发到 v2 legacy `/api/device-management*`），并为参考区调试接口 `/debug/latest-data*` 提供 501 disabled 占位，避免旧脚本 404。
