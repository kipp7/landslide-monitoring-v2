...
最后更新时间：2025-12-26（WS-N.21：参考区 app/hooks 1:1 入口补齐，PR #255；WS-K.17：legacy camera alias，PR #252；WS-K.16：legacy baselines alias routes，PR #251；WS-K.15：legacy compat aliases 补齐，PR #247；WS-N.20：/analysis legacy 占位组件还原）
...
- 2025-12-26: fix(api): legacy `/api/camera` `test_connection` response aligns with reference shape (`ip/http/websocket/stats/message`).
- `npm run lint`
- `npm run build`
