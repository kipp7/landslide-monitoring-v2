...
| WS-N.1 | codex | `feat/ws-n1/analysis-dashboard-core` |  | claimed | WS-K.2, WS-K.4, WS-K.3 | Web：恢复 legacy `/analysis` “运行大屏”核心信息架构（地图区 + 侧边栏 + 顶部状态），并将数据源切到 v2（SSE realtime + alerts + device state），确保旧展示能力不缺失；会改动 `apps/web/app/analysis/*` 与地图/状态组件，请勿并发修改这些文件 |
| WS-N.2 |  |  |  | backlog | WS-N.1 | Web：恢复 legacy 地图能力（聚合点/弹窗/风险颜色/一键切换地图模式），优先基于 v2 已依赖的 `leaflet/react-leaflet` 落地，避免引入需 key 的第三方地图服务 |
| WS-N.3 |  |  |  | backlog | WS-N.1, WS-K.4 | Web：恢复 legacy 实时异常表/实时传感器状态等组件（`RealtimeAnomalyTable`/`RealtimeSensorStatus*`），并对齐 v2 `/api/anomaly-assessment` + `/realtime/stream` 的数据结构 |
| WS-N.4 |  |  |  | backlog | WS-N.1, WS-L.1 | Web：恢复 legacy AI 预测卡片/图表组件（温湿度/加速度/陀螺仪等）在 `/analysis` 的入口与懒加载，数据源使用 v2 `/api/v1/data/series` 与 `/api/v1/ai/predictions*` |
