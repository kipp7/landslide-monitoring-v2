...
| WS-K.30 | codex | `fix/api-iot-server-compat-iot-prefix` |  | in_progress |  | API：为 iot-server-compat（`/info`、`/devices/*` 等）增加 `/iot/*` 前缀别名，兼容参考区常见 `apiBase=.../iot` 调用方式。 |
| WS-K.32 | codex | `fix/api-monitoring-stations-metadata-parity` |  | in_progress | WS-M.1 | API：补齐 legacy `monitoring-stations` 列表/详情的“配置回读”（从 `devices.metadata` 回填 `station_name`/`location_name`/`risk_level`/`sensor_types`/`chart_legend_name`/`status` 等字段），避免前端保存后刷新丢失。 |
