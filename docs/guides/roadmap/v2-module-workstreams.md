...
| WS-D.2 | codex | `feat/ws-d/deformation-trends` | https://github.com/kipp7/landslide-monitoring-v2/pull/107 | in_review | WS-D.1 | 仅做“API + 查询”闭环：新增 `/api/v1/gps/deformations/{deviceId}/series`（baseline + ClickHouse 遥测计算位移）；会改动 `services/api/src/routes/*` 与 `docs/integrations/api/openapi.yaml` |
| WS-D.3 | codex | `feat/ws-d/baseline-management-ui` |  | claimed | WS-D.1 | Web: 基准点管理页（`/device-management/baselines`）对接 `/api/v1/gps/baselines/*` |
