...
| WS-G.4 | codex | `fix/api-legacy-debug-tools` | https://github.com/kipp7/landslide-monitoring-v2/pull/new/fix/api-legacy-debug-tools | in_review | WS-G | API：补齐参考区调试端点 `/api/test-db`、`/api/inspect-*`、`/api/test-expert-health`（安全只读 + 权限控制），保留 `/api/db-admin` 禁用 |
