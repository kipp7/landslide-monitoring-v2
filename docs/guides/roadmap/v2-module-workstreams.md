...
| WS-K.15 | codex | `feat/ws-k15/legacy-compat-aliases` | https://github.com/kipp7/landslide-monitoring-v2/pull/247 | in_review | WS-M.1 | Legacy 优化/真实数据 API 别名：对齐参考区 `frontend/app/api/{device-management-optimized,device-management-real,device-management-real-db,monitoring-stations-optimized}`；v2 不复刻 Supabase 直连实现，而是提供别名转发到 v2 已有 legacy compat 端点 |
| WS-K.16 | codex | `feat/ws-k16/baseline-legacy-aliases` | https://github.com/kipp7/landslide-monitoring-v2/pull/251 | in_review | WS-K.5, WS-K.8 | Legacy baselines API 别名：对齐参考区 `frontend/app/api/baselines/[deviceId]/{auto-establish-advanced,auto-establish-simple,quality-assessment}`；v2 复用现有 `/auto-establish` 与 `/quality-check`，不引入 Supabase 直连 |
| WS-K.17 | codex | `feat/ws-k17/legacy-camera-alias` | https://github.com/kipp7/landslide-monitoring-v2/pull/252 | in_review | WS-K.1 | Legacy camera API 前缀兼容：对齐参考区 `frontend/app/api/camera` 的调用路径；在 v2 暴露 `/api/camera` 与 `/iot/api/camera`（保持现有 handler 逻辑不变） |
| WS-K.18 | codex | `feat/ws-k18/iot-server-compat` | https://github.com/kipp7/landslide-monitoring-v2/pull/254 | in_review | WS-M.1 | iot-server 兼容端点：对齐参考区通过 `IOT_SERVICE_BASE` 访问的 `/devices/*`（`/devices/mappings`、`/devices/:deviceId`、`/devices/list`、`/devices/info/:simpleId`、`/info`）；v2 转发到现有 legacy compat（`/api/iot/devices/*`），不引入 Supabase |
| WS-K.19 | codex | `feat/ws-k19/data-aggregation-clear-cache` | https://github.com/kipp7/landslide-monitoring-v2/pull/256 | in_review | WS-M.1 | Legacy data-aggregation 清缓存：对齐参考区 hook 调用的 `DELETE /api/data-aggregation?action=clear_cache`；v2 提供兼容 no-op 响应（避免旧前端报错） |
| WS-K.20 | codex | `feat/ws-k20/iot-prefix-huawei-health` | https://github.com/kipp7/landslide-monitoring-v2/pull/258 | in_review | WS-K.6, WS-K.10 | Legacy `/iot` 前缀补齐（华为/健康检查）：对齐参考区以 `baseUrl=${host}/iot` 调用 `/iot/health` 与 `/iot/huawei/*` 的情况；v2 将现有 `/huawei/*` 兼容端点额外挂载到 `/iot/huawei/*`，并补齐 `/iot/health` |
