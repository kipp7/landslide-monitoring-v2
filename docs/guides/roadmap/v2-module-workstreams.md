...
| WS-N.3 | codex | `feat/ws-n3/analysis-realtime-widgets` |  | claimed | WS-N.1, WS-K.4 | Web：恢复 legacy 实时异常表/实时传感器状态等组件（`RealtimeAnomalyTable`/`RealtimeSensorStatus*`），并对齐 v2 `/api/anomaly-assessment` + `/realtime/stream` 的数据结构 |
