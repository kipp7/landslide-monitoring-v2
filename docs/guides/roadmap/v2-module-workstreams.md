...
| WS-D.4 | codex | `feat/ws-d/gps-monitoring-ui` |  | claimed | WS-D.2, WS-D.3 | Web: 恢复 GPS 监测/形变页面（`/gps-monitoring` + `/gps-deformation`），对接 `/api/v1/data/series` 与 `/api/v1/gps/deformations/*` |
