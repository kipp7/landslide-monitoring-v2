...
| WS-N.10 | codex | `feat/ws-n10/analysis-device-naming` |  | claimed | WS-N.5 | Web：`/analysis` 设备命名/映射 1:1：对齐参考区 `device_mapping`/位置命名逻辑（generateDeviceName/getRiskByLocation/getDetailedLocationInfo），在 v2 侧实现等价映射（不直连 Supabase） |
