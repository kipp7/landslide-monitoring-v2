...
| WS-G.7 | codex | `fix/ws-g7-clickhouse-auth-fallback` | https://github.com/kipp7/landslide-monitoring-v2/pull/293 | done | WS-G | Infra：`init-clickhouse.ps1` 遇到 ClickHouse 认证失败时，读取容器内实际凭据并重试，避免本地长期环境与 `.env` 不一致导致 e2e/Stage5Regression 初始化失败 |
| WS-G.8 | codex | `fix/ws-g8-legacy-alias-duplicate-route` | https://github.com/kipp7/landslide-monitoring-v2/pull/295 | done | WS-G | API：修复 legacy compat 下 `/api/device-management-optimized` 等路由重复注册导致 Fastify 启动失败，恢复 e2e/Stage5Regression 启动链路 |
| WS-G.9 | codex | `fix/ws-g9-e2e-infra-runtime-creds` | https://github.com/kipp7/landslide-monitoring-v2/pull/296 | done | WS-G | Infra：e2e 生成服务 `.env` 时优先读取 compose 容器内实际 DB 凭据，避免本地长期环境与 `infra/compose/.env` 不一致导致 Stage5Regression 访问 `/api/v1/system/status`/`/dashboard` 失败 |
| WS-G.10 | codex | `fix/web-next-build-memory-r3` | https://github.com/kipp7/landslide-monitoring-v2/pull/302 | in_review | WS-G | Web/Tooling：Windows 下 `next build` 通过脚本补齐 `NODE_OPTIONS=--max-old-space-size=8192` 并用 `cmd.exe` 执行，降低 OOM / `spawn EINVAL` 失败概率 |
...
| WS-K.33 | codex | `fix/ws-k33-monitoring-stations-chart-config` | https://github.com/kipp7/landslide-monitoring-v2/pull/283 | done | WS-K.32 | API：完善 legacy `monitoring-stations` 的 `chartType` 图表配置返回（补齐 `title/unit/yAxisName/deviceLegends`）并增加 `/api/monitoring-stations/chart-config?type=...` 兼容端点，避免旧前端图表配置为空/缺字段。 |
| WS-K.34 | codex | `fix/api-camera-legacy-put-delete` | https://github.com/kipp7/landslide-monitoring-v2/pull/294 | in_review | WS-K.22 | API：补齐 legacy `PUT/DELETE /api/camera`（含 `/iot/api/camera`）对齐参考区 `frontend/app/api/camera/route.ts`，避免旧前端更新/删除摄像头时 404 |
