...
| WS-N.4 | codex | `feat/ws-n4/analysis-ai-widgets` |  | claimed | WS-N.1, WS-L.1 | Web：恢复 legacy AI 预测卡片/图表组件（温湿度/加速度/陀螺仪等）在 `/analysis` 的入口与懒加载，数据源使用 v2 `/api/v1/data/series` 与 `/api/v1/ai/predictions*` |
