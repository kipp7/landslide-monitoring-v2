...
| WS-K.10 | codex | `feat/ws-k10/iot-api-legacy-alias` |  | in_progress | WS-M.1 | Legacy nginx 前缀别名：对齐参考区/旧部署中 `/iot/api/*` 访问方式；在 v2 api-service 同时注册 `/iot/api/*` 与 `/api/*` 等价兼容层（并补齐 `/api/camera`，此前仅有 `/camera`） |
