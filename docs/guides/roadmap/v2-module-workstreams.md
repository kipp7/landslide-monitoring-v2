...
| WS-O.2 | codex | `feat/ws-o2/optimized-demo-legacy` | https://github.com/kipp7/landslide-monitoring-v2/pull/212 | in_review | WS-C, WS-M.1 | Web：`/optimized-demo` 1:1：恢复参考区“数据库优化演示”页面与依赖组件（`OptimizedDeviceStatus`、`useDataAggregation` 等），数据源切到 v2 API/legacy compat；当前 v2 `/optimized-demo` 仅重定向，需保留 v2 版到 `/optimized-demo-v2` |
...
| WS-O.4 | codex | `feat/ws-o4/device-management-legacy` | https://github.com/kipp7/landslide-monitoring-v2/pull/218 | in_review | WS-B, WS-D, WS-M.1 | Web：`/device-management` 1:1：恢复参考区设备管理页（状态/监测站管理/基准点管理 tabs），并将依赖组件（`MonitoringStationManagement`/`DeviceMappingTable` 等）迁移到 v2 数据源；保留现有 v2 设备管理页到 `/device-management-v2` |
