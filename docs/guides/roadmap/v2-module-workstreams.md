...
| WS-G.9 | codex | `fix/web-next-build-memory` | https://github.com/kipp7/landslide-monitoring-v2/pull/292 | in_review | WS-G | Web/Tooling：Windows 下 `next build` 通过脚本补齐 `NODE_OPTIONS=--max-old-space-size=8192` 并用 `cmd.exe` 执行，降低 OOM / `spawn EINVAL` 失败概率 |
