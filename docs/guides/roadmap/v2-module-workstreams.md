...
| WS-N.1 |  |  |  | backlog | WS-K.2, WS-K.4, WS-K.3 | Web：恢复 legacy `/analysis` “运行大屏”核心信息架构（地图区 + 侧边栏 + 顶部状态），并将数据源切到 v2（SSE realtime + alerts + device state），确保旧展示能力不缺失（UI 可按 v2 风格重做，不要求像素级一致） |
