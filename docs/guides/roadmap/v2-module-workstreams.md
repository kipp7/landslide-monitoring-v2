...
| WS-K.8 | codex | `feat/ws-k8/baselines-legacy-crud` |  | in_progress | WS-D.1, WS-K.5, WS-M.1 | Legacy baselines CRUD：对齐参考区 `backend/iot-service/baseline-management-api.js` 的 `/api/baselines`（列表/查询/创建/更新/删除）；在 v2 api-service `/api/baselines*` 补齐缺失端点（支持 legacy deviceId → UUID 映射） |
