...
| WS-G.8 | codex | `fix/ws-g8-legacy-alias-duplicate-route` | https://github.com/kipp7/landslide-monitoring-v2/pull/295 | in_review | WS-G | API：修复 legacy compat 下 `/api/device-management-optimized` 等路由重复注册导致 Fastify 启动失败，恢复 e2e/Stage5Regression 启动链路 |
| WS-G.9 | codex | `fix/web-next-build-memory-r3` | https://github.com/kipp7/landslide-monitoring-v2/pull/302 | in_review | WS-G | Web/Tooling：Windows 下 `next build` 通过脚本补齐 `NODE_OPTIONS=--max-old-space-size=8192` 并用 `cmd.exe` 执行，降低 OOM / `spawn EINVAL` 失败概率 |
