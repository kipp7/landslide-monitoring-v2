...
| WS-N.2 | codex | `feat/ws-n2/map-features` |  | claimed | WS-N.1 | Web：恢复 legacy 地图能力（聚合点/弹窗/风险颜色/一键切换地图模式），优先基于 v2 已依赖的 `leaflet/react-leaflet` 落地，避免引入需 key 的第三方地图服务；会改动 `apps/web/app/analysis/*` 地图组件，请勿并发修改这些文件 |
