...
| WS-G.5 | codex | `feat/api-db-admin-safe` | https://github.com/kipp7/landslide-monitoring-v2/pull/new/feat/api-db-admin-safe | in_progress | WS-G.4 | API：实现安全版 `/api/db-admin`（默认关闭 + 强权限 + 只读 query/analyze/backup；写操作继续禁用） |
