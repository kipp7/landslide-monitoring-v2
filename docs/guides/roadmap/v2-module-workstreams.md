...
| WS-K.24 | codex | `fix/api-gps-deformation-response-shape` | PR TBD | doing | WS-K.7, WS-K.21 | API：legacy `/api/gps-deformation/:deviceId` 响应结构对齐参考区（顶层 `deviceId/results/points/timestamp`），并让 v2 Web `/gps-monitoring` 兼容旧 `{ success, data }` 包装响应。 |
