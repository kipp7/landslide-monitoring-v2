...
| WS-K.11 | codex | `feat/ws-k11/legacy-iot-device-endpoints` | https://github.com/kipp7/landslide-monitoring-v2/pull/236 | in_review | WS-M.1, WS-K.10 | Legacy iot-server 顶层端点：对齐参考区 `backend/iot-service/iot-server.js` 的 `/info`、`/devices/*`、`/debug/latest-data/*`；在 v2 api-service 提供等价兼容实现（基于 Postgres 设备/站点与 ClickHouse 遥测），不依赖 Supabase/华为云直连 |
