...
| WS-G.3 | codex | `feat/ws-g3/ops-debug-api-smoke-tests` | https://github.com/kipp7/landslide-monitoring-v2/pull/221 | in_review | WS-G, WS-K.6 | Ops：`/ops/debug-api` 补齐参考区 debug-api 的“一键连通性测试”（`/health`、`/huawei/config`、`/huawei/devices/:deviceId/shadow`）；危险的 `POST /huawei/devices/:deviceId/*` 默认关闭并需要二次确认 |
