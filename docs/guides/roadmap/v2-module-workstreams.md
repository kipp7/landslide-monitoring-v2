...
| WS-K.15 | codex | `feat/ws-k15/legacy-optimized-real-alias` | https://github.com/kipp7/landslide-monitoring-v2/pull/253 | in_progress | WS-M.1, WS-K.11 | Legacy optimized/real 端点别名：对齐参考区 `frontend/app/api/{device-management-optimized,device-management-real,device-management-real-db,monitoring-stations-optimized}`；在 v2 legacy compat 层增加 307 别名跳转到对应的 `/api/device-management*` 与 `/api/monitoring-stations*`，避免老路径 404 |
