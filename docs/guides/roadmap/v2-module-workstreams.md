...
| WS-K.33 | codex | `fix/ws-monitoring-stations-chart-endpoints` |  | claimed | WS-K.32 | API：补齐 legacy `monitoring-stations` 的 `chart-config` 与 `chart-legends` 端点（图表 title/unit/yAxisName 映射 + `deviceLegends` 从 metadata 回读）。 |
