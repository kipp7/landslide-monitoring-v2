...
| WS-G.9 | codex | `fix/ws-g9-e2e-infra-runtime-creds` |  | in_progress | WS-G | Infra：e2e 生成服务 `.env` 时优先读取 compose 容器内实际 DB 凭据，避免本地长期环境与 `infra/compose/.env` 不一致导致 Stage5Regression 访问 `/api/v1/system/status`/`/dashboard` 失败 |
