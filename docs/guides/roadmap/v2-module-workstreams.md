...
| WS-B | codex | `feat/ws-b/station-detail-page` |  | in_progress |  | Web: station detail page (`/stations/{stationId}`) |
...
| WS-F | codex | `docs/ws-f/claim` |  | claimed |  | IoT adapter: Huawei IoT HTTP push -> Kafka telemetry (no UI) |
