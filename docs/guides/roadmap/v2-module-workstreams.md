...
| WS-K.5 | codex | `feat/ws-k5/gps-baselines-advanced` |  | claimed | WS-D.1 | GPS baselines 高级能力：对齐参考区 `/api/baselines/:deviceId/auto-establish`、`/quality-check`、`/available-devices`；在 v2 补齐等价能力（契约+实现+回归留证），并确认 Web 基准点管理页的入口/字段一致性 |
