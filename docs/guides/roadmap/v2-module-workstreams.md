...
| WS-G.8 | codex | `fix/ws-g8-legacy-alias-duplicate-route` |  | in_progress | WS-G | API：修复 legacy compat 下 `/api/device-management-optimized` 等路由重复注册导致 Fastify 启动失败，恢复 e2e/Stage5Regression 启动链路 |
