...
| WS-K.7 | codex | `feat/ws-k7/gps-deformation-legacy-compat` |  | in_progress | WS-D.2 | Legacy GPS 形变端点：对齐参考区 `/api/gps-deformation/:deviceId`，在 v2 增加兼容层（支持 legacy deviceId → UUID 映射；查询 ClickHouse + baseline 计算位移/速度序列；不实现参考区 CEEMD/AI 分析结果） |
