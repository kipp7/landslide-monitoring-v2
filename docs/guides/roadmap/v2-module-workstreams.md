...
| WS-K.33 | codex | `fix/ws-k33-monitoring-stations-chart-config` |  | in_progress | WS-K.32 | API：完善 legacy `monitoring-stations` 的 `chartType` 图表配置返回（补齐 `title/unit/yAxisName/deviceLegends`）并增加 `/api/monitoring-stations/chart-config?type=...` 兼容端点，避免旧前端图表配置为空/缺字段。 |
