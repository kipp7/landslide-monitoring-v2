...
| WS-M.1 | codex | `feat/ws-m1/legacy-device-management-api` |  | claimed | WS-A | Legacy 设备管理 API 兼容：对齐 `legacy-frontend/app/api/device-management*`、`/monitoring-stations*`、`/iot/devices*`、`/data-aggregation` 的关键返回结构；v2 以 `/api` 兼容层落地，并尽量映射到既有 v1 设备/站点/数据/基准点/告警能力 |
