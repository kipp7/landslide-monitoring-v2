...
| WS-K.10 | codex | `feat/ws-k10/iot-api-legacy-alias` |  | in_progress | WS-M.1, WS-K.1, WS-K.7, WS-K.8 | Nginx `/iot` 前缀兼容：补齐 legacy `/iot/api/*` 等价别名（对齐参考区通过 `/iot/api/*` 访问旧后端的方式），并补齐 `/iot/api/camera`（此前仅注册了 `/camera`） |
