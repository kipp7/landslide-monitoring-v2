...
| WS-K.28 | codex | `fix/api-monitoring-stations-deviceid-mutations` | https://github.com/kipp7/landslide-monitoring-v2/pull/273 | in_review | WS-M.1 | API：补齐 legacy `/api/monitoring-stations/{deviceId}` 的 `PUT`/`DELETE`（参考区 `frontend/app/api/monitoring-stations/[deviceId]/route.ts`）。 |
