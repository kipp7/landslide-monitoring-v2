...
| WS-K.25 | codex | `fix/web-api-proxy-backend-url-fallback` | PR TBD | doing | WS-K.22 | Web：Next API proxy（`apps/web/app/api/_proxy.ts`）补齐 `BACKEND_URL`/`NEXT_PUBLIC_BACKEND_URL` 的 baseUrl fallback，兼容参考区部署环境只配置 BACKEND_URL 的场景。 |
