...
| WS-N.5 | codex | `feat/ws-n5/analysis-legacy-shell` |  | claimed | WS-N.1~WS-N.4 | Web：`/analysis` UI 1:1 还原（骨架）：移植参考区 `HoverSidebar`、`BaseCard`、`MapSwitchPanel`、`LazyComponents` 框架与布局结构（先确保 DOM/布局/交互入口一致，数据源适配后续拆分） |
