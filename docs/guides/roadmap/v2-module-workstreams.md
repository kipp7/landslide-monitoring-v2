...
| WS-D.2 | codex | `feat/ws-d/deformation-trends2` | https://github.com/kipp7/landslide-monitoring-v2/pull/110 | in_review | WS-D.1 | 仅做“API + 查询”闭环：新增 `/api/v1/gps/deformations/{deviceId}/series`（baseline + ClickHouse 遥测计算位移）；会改动 `services/api/src/routes/*` 与 `docs/integrations/api/openapi.yaml` |
