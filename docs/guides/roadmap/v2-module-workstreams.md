...
| WS-K.34 | codex | `fix/ws-k34-legacy-camera-put-delete` |  | in_progress | WS-K.1 | API：补齐参考区 `frontend/app/api/camera/route.ts` 的 legacy `PUT/DELETE /api/camera`（更新设备信息 + 删除设备），并支持在线时透传 `POST http://{ip}/api/config`。 |
