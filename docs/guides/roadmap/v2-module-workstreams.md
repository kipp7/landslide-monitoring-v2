...
| WS-N.7 | codex | `feat/ws-n7/analysis-charts-rest` |  | claimed | WS-N.5 | Web：`/analysis` 图表 1:1（其余图表）：移植液位图（LiquidFill）、柱状图（BarChart）、设备错误图（DeviceErrorChart）、异常类型图（AnomalyTypeChart）并适配 v2 数据/告警聚合 |
