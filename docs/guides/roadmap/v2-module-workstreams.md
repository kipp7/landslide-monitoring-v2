...
| WS-K.13 | codex | `feat/ws-k9/legacy-device-management-tools` |  | in_progress | WS-M.1, WS-K.10 | Legacy 设备管理工具端点：对齐参考区 `frontend/app/api/device-management/{export,reports,diagnostics}`；在 v2 `/api/device-management/*` 增加兼容实现（ClickHouse/Postgres 数据源；禁用参考区的 Supabase/外部 AI 直连） |
