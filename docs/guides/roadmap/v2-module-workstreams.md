...
| WS-G.10 | codex | `fix/web-next-build-memory-r3` | https://github.com/kipp7/landslide-monitoring-v2/pull/302 | done | WS-G | Web/Tooling：Windows 下 `next build` 通过脚本补齐 `NODE_OPTIONS=--max-old-space-size=8192` 并用 `cmd.exe` 执行，降低 OOM / `spawn EINVAL` 失败概率 |
...
| WS-K.34 | codex | `fix/api-camera-legacy-put-delete` | https://github.com/kipp7/landslide-monitoring-v2/pull/294 | done | WS-K.22 | API：补齐 legacy `PUT/DELETE /api/camera`（含 `/iot/api/camera`）对齐参考区 `frontend/app/api/camera/route.ts`，避免旧前端更新/删除摄像头时 404 |
